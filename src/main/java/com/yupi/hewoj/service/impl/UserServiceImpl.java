...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        synchronized (userAccount.intern()) {
            ...
            if (count > 0) {
                throw new BusinessException(ResponseCodeEnum.PARAMS_ERROR, "账号重复");
            }
            // 2. 加密
            ...
            if (!saveResult) {
                throw new BusinessException(ResponseCodeEnum.SYSTEM_ERROR, "注册失败，数据库错误");
            }
            return user.getId();
        }
    }
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        if (userAccount.length() < 4) {
            throw new BusinessException(ResponseCodeEnum.PARAMS_ERROR, "账号错误");
        }
        if (userPassword.length() < 8) {
            throw new BusinessException(ResponseCodeEnum.PARAMS_ERROR, "密码错误");
        }
        // 2. 加密
        String encryptPassword = DigestUtils.md5DigestAsHex((SALT + userPassword).getBytes());
        // 查询用户是否存在
        ...
        if (user == null) {
            ...
            throw new BusinessException(ResponseCodeEnum.PARAMS_ERROR, "用户不存在或密码错误");
        }
        // 3. 记录用户的登录态
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        if (currentUser == null || currentUser.getId() == null) {
            throw new BusinessException(ResponseCodeEnum.NOT_LOGIN_ERROR);
        }
        // 从数据库查询（追求性能的话可以注释，直接走缓存）
//        long userId = currentUser.getId();
//        User currentUser1 = this.getById(userId);
...
//        }
    ...
    public boolean userLogout(HttpServletRequest request) {
        if (request.getSession().getAttribute(USER_LOGIN_STATE) == null) {
            throw new BusinessException(ResponseCodeEnum.OPERATION_ERROR, "未登录");
        }
        // 移除登录态
    ...
    public QueryWrapper<User> getQueryWrapper(UserQueryRequest userQueryRequest) {
        if (userQueryRequest == null) {
            throw new BusinessException(ResponseCodeEnum.PARAMS_ERROR, "请求参数为空");
        }
